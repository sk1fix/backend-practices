...
@conversion_route.get("/conversion", summary="Get conversion", tags=["Conversion Endpoints"])
async def get_conversion(
        base: str = Query(alias='from'),
        target: str = Query(alias='to'),
        amount: float = Query(alias='amount'),
        service: CurrencyConversionService = Depends(get_conversion_service)) -> CurrencyConversionDto:
